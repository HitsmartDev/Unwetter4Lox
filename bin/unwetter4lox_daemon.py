"""Unwetter4Lox Daemon v0.4.33 – GeoSphere (ZAMG) + INCA + TAWES 360° -> MQTT
   Changelog:
   v0.4.33: Safe-Boot Version: datetime.fromisoformat entfernt (Inkompatibilität < Python 3.7).
            Umfangreiches Exception-Logging via traceback hinzugefügt.
            Dictionary-Handling beim Start robuster gestaltet.
   v0.4.32: Historischer Daten-Abruf (letzte 60min) beim Start implementiert.
   v0.4.31: MQTT LWT (Last Will and Testament) implementiert.
"""
...
def _on_connect(c, u, f, rc):
    if rc == 0: _mqtt_connected.set(); log.info(f'MQTT: Verbunden mit {MQTT_BROKER}:{MQTT_PORT}')
    else: log.error(f'MQTT: Verbindung fehlgeschlagen (RC={rc})')
...
def mqtt_connect():
    ...
    try:
        ...
        lwt_topic = f'{TOPIC_PREFIX}/status'; lwt_msg = L['status_err'].format(v='Offline (LWT)')
    ...
    except Exception as e: log.error(f'MQTT: Fehler: {e}'); return False
...
def publish(topic, value, retain=True):
    if mqtt_client and _mqtt_connected.is_set():
        ...
        except Exception as e: log.debug(f'MQTT: Publish fehlgeschlagen ({topic}): {e}')

def fetch_json(url, provider):
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'Unwetter4Lox/0.4.33'})
...
def _parse_iso(ts):
    ...
    try:
        # '2026-06-13T12:00:00Z' or '2026-06-13T12:00'
...
def run():
    log.info(f'Unwetter4Lox v0.4.33 gestartet | Interval={INTERVAL}s')
    try:
        with open(PID_FILE, 'w') as f: f.write(str(os.getpid()))
    except: pass
    
    # Initialsierung Status
    ...
    while True:
        try:
            ...
            first = False
        except Exception: log.error(f'Loop Fehler: {traceback.format_exc()}')
        time.sleep(INTERVAL)
