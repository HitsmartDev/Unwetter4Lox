...
## [0.9.0] – 2026-06-17 – Erstes öffentliches Release

### Hinzugefügt
- **TAWES 360° Stationsnetz**: Automatische Erkennung von Upstream-Stationen basierend auf der vorherrschenden Windrichtung. Böen- und Regenwarnungen aus ~360° des lokalen Wetternetzes.
- **Lokal-Regen-Erkennung**: Stationen bis 25 km (Alarm) bzw. 40 km (Anzeige) überwacht für Echtzeit-Regendetection.
- **MQTT-Robustheit**: Watchdog erkennt tote Verbindungen und erzwingt Reconnect. LWT (Last Will) wird sofort nach Reconnect überschrieben. Detailliertes Logging bei Trennung inkl. RC-Code.
- **Automatischer Neustart**: Täglicher Daemon-Restart um 03:00 Uhr und automatischer Start nach LoxBerry-Neustart via `/etc/cron.d/`.
- **Alarm-Level-Schema**: 0 = Ruhig, 1 = Vorsicht, 2 = Warnung, 3 = Extrem – aus ZAMG, INCA und TAWES aggregiert.
- **Notification-Topics**: `notification/inca`, `notification/tawes`, `notification/alle`, `notification/geosphere` für Push-Benachrichtigungen in Loxone.
- **Wind-Kaskaden-Erkennung**: Zeitliche Abfolge von Böen in Windrichtung → ETA-Berechnung.
- **Mehrsprachigkeit**: Deutsch und Englisch (automatisch via LoxBerry-Spracheinstellung).

### Technisches
- GeoSphere Austria API (ZAMG-Warnungen, INCA Nowcast, TAWES Stationsdaten)
- MQTT mit LWT, retained Messages, konfigurierbarem Topic-Prefix
- LoxBerry-Standard-Logging mit Log-Viewer-Integration
- Konfigurierbar über LoxBerry-Plugin-UI

---

## Frühere Versionen (interne Entwicklungshistorie)

### Behoben (2026-06-13)
- Kompatibilität mit älteren Python-Versionen: `datetime.fromisoformat` entfernt (erst ab Python 3.7 verfügbar).
- Fehlerdiagnose: Crash-Logging in `crash.log` bei fatalen Fehlern.
- Historische Daten-Initialisierung: TAWES-Daten der letzten 60 Minuten werden beim Start geladen.
- MQTT LWT (Last Will and Testament) implementiert.
